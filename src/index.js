...
module.exports = function({ commitsPerDay, workdaysOnly, startDate, endDate }) {
  const commitDateList = createCommitDateList({
    workdaysOnly:true,
    commitsPerDay: [0,5],
    startDate: startDate ? parse(startDate) : addYears(new Date(), -7),
  ...
  (async function() {
    ...
    for (const date of commitDateList) {
      ...
      await execAsync(`git commit --quiet --date "${date}" -m "Godric Commit"`);
    }
...
function createCommitDateList({
  ...
  while (currentDate <= endDate) {
    if (workdaysOnly && isWeekend(currentDate) && Math.random()*10 < 8 ) {
    ...
    // let addDaysCnt =  Math.random()*10 > 8 ? Math.floor(Math.random()*3) : 1
    currentDate = addDays(currentDate, 1);
  }
